...
Build a histogram of alternative allele frequencies for this data set.

```{r }

allele_frequencies <- snp_data %>% 
  group_by(
    position
  ) %>% 
  summarize(
    alternative_allele_frequency = 100 * sum(genotype) / (2 * n())
  )
...
ggplot() +
  geom_histogram(
    ...
    mapping = aes(
      x = alternative_allele_frequency
    ),
    col = "darkblue",
    fill = "darkblue",
    alpha = 0.1,
...
The alternative allele frequencies in this data set are comprised between `r min(allele_frequencies$alternative_allele_frequency)` % and `r max(allele_frequencies$alternative_allele_frequency)` %.

_Would you filter out some of these variants prior to analysis?_
