...
The animation can be started using _animate()_ or saved to file using _anim_save()_. Note that the computation of the animation requires quite some memory and might not work on a standard desktop computer, running the code below on this example failed on a server with ~64GB of RAM.
...
In this section, we draw an animated Manhattan plot based on whole genome variants. In the previous section, we were already limited by the number of points in the release of the animation, here each plot contains several millions of points, so we will look towards an alternative solution. 

In this example, we use the association results with BMI over the first years of life. We combine the figure below (Supplementary Figure 1 of [this preprint](https://www.biorxiv.org/content/early/2018/11/25/478255)) into a single animation.

![Supp Fig 1](plots/FigS1.png "Supp Fig 1")

In order to keep the memory consumption as low as when making the figure above, we compute export the different frames one by one on the fly. This is made simple by the fact that Manhattan plots of the same markers all display the same points, we just need to move them up and down. While we iterate the different frames, we build a command line that will use [magick](https://www.imagemagick.org/script/index.php) to build the gif. The code below shows how the animation was made, note that I was not allowed to share the code I wrote to produce the figure above or underlying data, so here is only the part that builds the animation. I will be happy to answer questions and comments on the [issue tracker](https://github.com/mvaudel/tutorials/issues).
